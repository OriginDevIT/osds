...
4. **No data-source connectors.** OSDS ships no importer, scraper, hook, or plugin interface for any external listing dataset. This is a legal position, not a missing feature. See `docs/spec/` §3.1.1 and `CONTRIBUTING.md`. Contributions adding one are declined on sight - say so politely and link the section.
